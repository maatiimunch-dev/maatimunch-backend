...
const createCategory = async (req, res) => {
  try {
    ...
    if (existingCategory) return res.status(409).json({ success: false, message: 'Category already exists' });
    ...
    console.error('❌ Error creating category:', error);
    res.status(500).json({ success: false, message: `Cannot create category: ${error.message}` });
  }
};

// Get All Categories (cache applied via route middleware)
const getAllCategories = async (req, res) => {
  try {
    const { isActive, includeProductCount } = req.query;
    let filter = {};
    ...
    if (includeProductCount === 'true') {
      categories = await Category.aggregate([
        ...
        { $project: { name: 1, description: 1, isActive: 1, createdAt: 1, updatedAt: 1, productCount: 1 } },
        { $sort: { createdAt: -1 } },
      ]);
    } else {
      categories = await Category.find(filter).sort({ createdAt: -1 });
    ...
    console.error('❌ Error fetching categories:', error);
    res.status(500).json({ success: false, message: 'Cannot fetch categories', error: error.message });
  }
};
...
const getCategoryById = async (req, res) => {
  try {
    ...
    const productCount = await Product.countDocuments({ category: category.name });
    res.status(200).json({ success: true, data: { ...category.toObject(), productCount } });
  } catch (error) {
    res.status(500).json({ success: false, message: 'Cannot fetch category', error: error.message });
  }
};
...
const updateCategory = async (req, res) => {
  try {
    ...
    if (name && name.trim() !== category.name) {
      ...
      const oldCategoryName = category.name;
      const newCategoryName = name.trim();
      await Product.updateMany({ category: oldCategoryName }, { category: newCategoryName });
      console.log(`✅ Updated products from "${oldCategoryName}" to "${newCategoryName}"`);
      category.name = newCategoryName;
    }
    ...
    res.status(500).json({ success: false, message: `Cannot update category: ${error.message}` });
  }
};
...
const deleteCategory = async (req, res) => {
  try {
    ...
    if (productCount > 0) {
      if (reassignTo) {
        const newCategory = await Category.findById(reassignTo);
        if (!newCategory) return res.status(400).json({ success: false, message: 'Invalid reassign category ID' });
        await Product.updateMany({ category: category.name }, { category: newCategory.name });
        console.log(`✅ Reassigned ${productCount} products to "${newCategory.name}"`);
      } else {
        return res.status(400).json({
          success: false,
          message: `Cannot delete category. ${productCount} products are using it. Provide a reassignTo category ID.`,
          productCount,
        });
      }
    }
    ...
    res.status(500).json({ success: false, message: `Cannot delete category: ${error.message}` });
  }
};
...
const toggleCategoryStatus = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: `Cannot toggle category status: ${error.message}` });
  }
};
...
const getActiveCategories = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Cannot fetch active categories', error: error.message });
  }
};
