...
const processAndUploadImage = async (file) => {
  try {
    ...
    if (megapixels > MAX_IMAGE_RESOLUTION_MP) {
      console.log(`🟠 Resizing (${megapixels.toFixed(2)}MP)...`);
      const aspectRatio = metadata.width / metadata.height;
      const targetPixels = MAX_IMAGE_RESOLUTION_MP * 1000000;
...
const createBlog = async (req, res) => {
  try {
    ...
    const uploadedImage = await processAndUploadImage(req.file);
    const blog = await Blog.create({ title, description, image: uploadedImage });
    ...
    console.error('🔴 Error creating blog:', error);
    res.status(500).json({ success: false, message: `Cannot create blog: ${error.message}` });
  }
};

// ─── Fetch All Blogs (cache via route middleware) ─────────────────────────────
...
const fetchBlogs = async (req, res) => {
  try {
    ...
    console.error('❌ Error fetching blogs:', error);
    res.status(500).json({ success: false, message: 'Cannot fetch blogs', error: error.message });
  }
};
...
const fetchSingleBlog = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Cannot fetch blog', error: error.message });
  }
};
...
const editBlog = async (req, res) => {
  try {
    ...
    if (req.file) blog.image = await processAndUploadImage(req.file);
    if (title) blog.title = title;
    if (description) blog.description = description;
    ...
    console.error('🔴 Error updating blog:', error);
    res.status(500).json({ success: false, message: `Cannot update blog: ${error.message}` });
  }
};
...
const deleteBlog = async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Cannot delete blog', error: error.message });
  }
};

module.exports = { createBlog, fetchBlogs, fetchSingleBlog, fetchBlogBySlug, editBlog, deleteBlog };